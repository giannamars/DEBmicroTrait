...
biomass_turnover!(J_B::Vector{Float64}, p::AbstractTurnover, B::Vector{Float64}, t::String) = begin
    J_B = γ_V_0(p).*B
end
...
reserve_recycling!(J_ED::Vector{Float64}, p::AbstractTurnover, E::Vector{Float64}, t::String) = begin
    J_ED = f_ED(p).*sum(γ_V_0(p).*E, dims=1)
end

biomass_recycling!(J_VD::Vector{Float64}, p::AbstractTurnover, V::Vector{Float64}) = begin
    J_VD = f_V(p).*f_VD(p).*sum(γ_V_0(p).*V./(γ_V_1(p) .+ V), dims=1)
    J_VP = f_VP(p).*sum((1 .- f_V(p)).*sum(γ_V_0(p).*V./(γ_V_1(p) .+ V), dims=1), dims=1)
    return J_VD, J_VP
end

biomass_recycling!(J_VD::Vector{Float64}, p::AbstractTurnover, V::Vector{Float64}, t::String) = begin
    J_VD = f_V(p).*f_VD(p).*sum(γ_V_0(p).*V, dims=1)
    J_VP = f_VP(p).*sum((1 .- f_V(p)).*sum(γ_V_0(p).*V, dims=1), dims=1)
    return J_VD, J_VP
end
