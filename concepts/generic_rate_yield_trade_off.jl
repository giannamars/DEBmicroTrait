...
function r_analytic_plus(k_E, y_EV, k_M, y_EM, y)
    return @. -k_E/2*(y_EV*y-1)*(1+sqrt(1-(4*k_E*k_M*y_EM*y)/((k_E*(y_EV*y-1))^2)))
end

function r_approx_plus(k_E, y_EV, k_M, y_EM, y)
    return @. -k_E*(y_EV*y-1) + 2*k_E*k_M*y_EM*y/(k_E*(y_EV*y-1))
end

function r_analytic_minus(k_E, y_EV, k_M, y_EM, y)
    return @. -k_E/2*(y_EV*y-1)*(1-sqrt(1-(4*k_E*k_M*y_EM*y)/((k_E*(y_EV*y-1))^2)))
end

yields = convert(Array{Float64,1}, LinRange(0.1, 0.989, 100))
rate_plus = r_analytic_plus(k_E[1], y_EV[1], k_M[1], y_EM[1], yields)
rate_approx_plus = r_approx_plus(k_E[1], y_EV[1], k_M[1], y_EM[1], yields)
rate_minus = r_analytic_minus(k_E[1], y_EV[1], k_M[1], y_EM[1], yields)
layer1 = layer(x=yields, y=rate_plus)
layer2 = layer(x=yields, y=rate_approx_plus)
layer3 = layer(x=yields, y=rate_minus)
plot(layer1, layer2, Scale.y_log10)


function max_yield_plus(k_E, y_EV, k_M, y_EM)
    return @. 1/y_EV*(1+sqrt(2*k_M*y_EM/(k_E*y_EV)))
end

max_yield_plus(k_E[1], y_EV[1], k_M[1], y_EM[1])

using QuadGK

function arc_length(k_E, y_EV, k_M, y_EM, y)
    return sqrt(1 + (-k_E*y_EV*(y*y_EV-1)^2-2*k_M*y_EM)^2)
end

arc_length_quad(y) = arc_length(k_E[1], y_EV[1], k_M[1], y_EM[1], y)
quadgk(arc_length_quad, 0, 1, rtol=1e-8)

yield = zeros(size(V_cs,1), 100000)
rate = zeros(size(V_cs,1), 100000)
ρs = zeros(size(V_cs,1))
als = zeros(size(V_cs,1))
...
for i in 1:100
    ...
    Yrmax = yield[i,rid]
    arc_length_quad(y) = arc_length(k_E[i],y_EV[i],k_M[i],y_EM[i],y)
    al, at = quadgk(arc_length_quad, Yrmax, Ymax, rtol=1e-8)
    als[i] = al
end

layer1 = layer(x=k_E, y=als)
layer2 = layer(x=y_EV, y=als)
plot(layer1, layer2)

plot(x=k_E, y=y_EV, Scale.x_log10, Scale.y_log10)

rate1 = filter!(x->x>0.0,rate[1,:])
yield1 = filter!(x->x>0.0,yield[1,:])

plot(x=rate1, y=yield1, Scale.y_log10, Scale.x_log10)


yield = zeros(size(V_cs,1), 600)
rate = zeros(size(V_cs,1), 600)
ρs = zeros(size(V_cs,1))
mEs = zeros(size(V_cs,1), 600)
...
layer3 = layer(x=yield_56, y=rate_56)
plot(layer1, layer3, Scale.y_log10)
